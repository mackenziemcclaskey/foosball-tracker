...
app.get("/api/players", async (_req, res, next) => {
  try {
    const players = (await dbStore.getPlayers())
      .map(toPublicPlayer)
...
app.post("/api/players", async (req, res, next) => {
  try {
    ...
    const existingPlayers = await dbStore.getPlayers();
    ...
    await dbStore.addPlayer(player);
...
app.post("/api/teams/suggest", async (req, res, next) => {
  try {
    ...
    const players = await Promise.all(playerIds.map((id) => dbStore.getPlayer(String(id))));
    if (players.some((p) => !p)) {
...
app.post("/api/matches", async (req, res, next) => {
  try {
    ...
    const [p1a, p1b, p2a, p2b] = await Promise.all(allIds.map((id) => dbStore.getPlayer(String(id))));
    if (!p1a || !p1b || !p2a || !p2b) {
      res.status(404).json({ error: "one or more players not found" });
      return;
    }
    ...
    await dbStore.recordMatch(match, updated);
...
app.get("/api/matches", async (req, res, next) => {
  try {
    ...
    const namesById = new Map((await dbStore.getPlayers()).map((p) => [p.id, p.name]));
    const matches = (await dbStore.getMatches(limit)).map((m) => ({
      ...m,
